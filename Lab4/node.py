...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for i in range(len(possible_splits)):
            left_side=y[:possible_splits[i]+1]
            right_side=y[possible_splits[i]+1:]
            left_pos=0
            left_neg=0
            right_pos=0
            right_neg=0
            ...
            gini_right=1-(right_pos/(right_pos+right_neg))**2 - (right_neg/(right_neg+right_pos))**2
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if(feature_subset!=None):
            ...
            picked_features = random.sample(range(X.shape[1]),feature_subset)
        else:
            picked_features=range(X.shape[1])


        for d in picked_features:
